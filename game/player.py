...
class Player:
    ...
    def possible_right(self, walls: np.ndarray, opponent: Player) -> bool:
        """This function tests if it is possible to go right"""
        board_size = len(walls) + 1
        return (self.j<board_size-1 and 
                walls[self.i, self.j] == 0 and 
                (self.i == 0 or walls[self.i-1,self.j]==0) and
                (self.i == opponent.i and self.j != opponent.j - 1))

    def possible_left(self, walls: np.ndarray, opponent: Player) -> bool:
        """This function tests if it is possible to go left"""
        return (self.j>0 and 
                walls[self.i, self.j-1] == 0 and 
                (self.i == 0 or walls[self.i-1,self.j-1]==0) and
                (self.i == opponent.i and self.j != opponent.j + 1))

    def possible_up(self, walls: np.ndarray,opponent: Player) -> bool:
        """This function tests if it is possible to go upward"""
        return (self.i>0 and 
                walls[self.i-1, self.j] == 0 and 
                (self.j == 0 or walls[self.i-1,self.j-1]==0) and
                (self.i != opponent.i-1 and self.j == opponent.j))

    def possible_down(self, walls: np.ndarray, opponent: Player) -> bool:
    ...
    def is_opponent_right(self, walls:np.ndarray, opponent:Player) -> bool:
    ...
    def possible_jump_diag_dr(self, walls_h: np.ndarray, walls_v:np.ndarray, opponent: Player) -> bool:
    ...
    def possible_jump_diag_ul(self, walls_h: np.ndarray, walls_v:np.ndarray, opponent: Player) -> bool:
    ...
    def possible_jump_diag_dl(self, walls_h: np.ndarray, walls_v:np.ndarray, opponent: Player) -> bool:
